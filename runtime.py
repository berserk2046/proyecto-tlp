...
class Juego:
        ...
        if nombre_evento in self.datos_juego['events']:
            for accion in self.datos_juego['events'][nombre_evento]:
                ...
                if self.tipo_juego == 'SNAKE':
                    ...
                    if verbo == 'SPAWN' and objeto == 'FOOD':self.snake_spawn_food('FOOD')
                                            
                    if verbo == 'SPAWN' and objeto == 'BFOOD':self.snake_spawn_food('BFOOD')
    
                    if verbo == 'SPAWN' and objeto == 'YFOOD':self.snake_spawn_food('YFOOD')
    ...
    def snake_spawn_food(self, tipo):
        self.spawn_shape('COMIDA', 'RANDOM')

        food = self.entities['COMIDA']['COMIDA'+str(self.entity_counter-1)]
